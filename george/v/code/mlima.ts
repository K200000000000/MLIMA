...
export class mlima extends page {
    ...
    */
    async display_message(
        ...
        const messageDiv: HTMLElement = this.create_element('div', message_element, {
        ...
        const message_output: HTMLElement | null = document.getElementById("message_output");
        //
    ...
    //Responsible for toggling back to the constitution
    async copy1(source_id:string, dest_id:string, file:string):Promise<void>{
      //
      //Create an instance of the template class, to support carnibalization
      const Template = new template(file);
      //
    ...
    //Responsible for toggling back to the constitution
    async show_katiba1():Promise<void>{
        this.copy('source', 'content', 'template.html');
    }
    ...
    sendMessage(): void {
        ...
        // Clear the text area after sending the message
        const messageTextArea: HTMLTextAreaElement | null = document.getElementById('message_text_area') as HTMLTextAreaElement;
        if (messageTextArea) {
